...
<nav id="navbar-main" class="navbar is-fixed-top">
  ...
  <div class="navbar-menu" id="navbar-menu">
    <div class="navbar-end">
      ...
      <div class="navbar-item dropdown has-divider has-user-avatar">
        <a class="navbar-link">
          ...
          <div class="user-avatar">
            <img src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" alt="{{ Auth::user()->name }}" class="rounded-full">
          </div>
          @endif
          <div class="is-user-name uppercase"><span >{{ Auth::user()->name }}</span></div>
          <span class="icon"><i class="mdi mdi-chevron-down"></i></span>
        </a>
        <div class="navbar-dropdown">
          <a href="{{ route('profile.show') }}" :active="request()->routeIs('profile.show')" class="navbar-item">
      ...
      <form method="POST" style="margin-top: 2%" action="{{ route('logout') }}" x-data>
        @csrf
        <a href="{{ route('logout') }}"  @click.prevent="$root.submit();" title="Log out" class="navbar-item desktop-icon-only" >
...
<aside class="aside is-placed-left is-expanded">
  <div class="aside-tools">
    <div>
       <b class="font-black">Octalink Global Sourcing</b>
    </div>
  </div>
